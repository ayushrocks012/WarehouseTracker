...
## Directory structure
- `SharedSource.pq` – single source of truth for SharePoint connection settings and helpers reused across queries (latest file + CSV/sheet loaders are already buffered to reduce re-scans, and a `WithFileNameColumn` helper surfaces the source file name for traceability).
...
1. **Shared foundation**: `power-query/SharedSource.pq` (update its `SiteUrl`/SharePoint settings before importing dependents).
